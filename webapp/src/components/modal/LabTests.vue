    ...
    <div class="modal-dialog"
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <p id="select-type-labs">Select the type of test and their result</p>
          <v-select :value.sync="selected"
                    label="name"
                    :options="options"
                    :on-change="updateResults"
                    placeholder="Search Lab Tests" class=""></v-select>
          <button v-for="answer in answers"
                  v-on:click="addLabTest(answer.id)"
                  class="btn btn-primary answer-type">{{answer.type}}
          </button>
    ...
    methods: {
      submitLabTests: function () {
        for (let answer of this.answersSubmmited) {
          this.$http.post('http://localhost:3000/api/labtests', {
            medicalId: answer.id,
            value: answer.value
          }, {headers: {Authorization: this.$root.key}})
